...
public class UIExploreChar : CharSkeletonOperation
{
    ...
    public void PlayAction(CharModuleAction charModuleAction, bool isAutoIdle = false)
    {
        ...
        if (isAutoIdle) trackentry = ActionPlayEnd;
    ...
    {
        //先移动到7/8的位置上
        var tempPos = Vector3.right * (800 / 2f) + Vector3.up * initPos.y;
        var temp = charTrans.localScale.y;
        ...
        //执行方法
        if (action != null) action(charTrans.localPosition.x - position.x);
        //重置到初始位置
    ...
    {
        var x = Vector3.one * charTrans.localScale.y;
    ...
    {
        if (IEUpdateMove != null) IEUpdateMove.Stop();
    ...
    private IEnumerator UpdateMove(Vector3 position, bool isTrapEvent, Action action, float moveTime)
    {
        ...
        var x = Math.Abs(Vector3.SqrMagnitude(charTrans.localPosition - leftEndVector3));
        ...
        float time = 0;
        if (moveTime == 0) isVisiteEventMoveEnd = true;
        while (Vector3.SqrMagnitude(charTrans.localPosition - leftEndVector3) >= 100)
        {
            if (aspd == 150 || aspd == 300) aspd = time < moveTime ? 150 : 300;
    ...
    public float StartCharMove(Vector3 movePos, float sp, float eventWidth, out float moveDistance)
    {
        ...
        {
            var temp = charTrans.localPosition;
            temp.x = 0;
    ...
    private void ResetMove()
    {
        ...
        if (Vector3.SqrMagnitude(charTrans.localPosition - startPos) >= 100) return;
