...
public class LoopScroller: MonoBehaviour
{
    ...
    /// <param name="onOutScrollAction">离开scroll回掉 可以不传</param>
    /// <param name="onInScrollAction">进入scroll回掉 可以不传</param>
    ...
    public Vector3 GetPosition(int i)
    {
        switch(Movement)
        {
            case Arrangement.Horizontal:
                ...
                return new Vector3(CellWidth * indexX + indexX * CellPadidingX,
                   -(CellHeight + CellPadidingY) * (i % MaxPerLine),
                   0f);
            case Arrangement.Vertical:
                int indexOfX = i % MaxPerLine;
                return new Vector3(CellWidth * indexOfX + indexOfX * CellPadidingX,
                  -(CellHeight + CellPadidingY) * (i / MaxPerLine),
                  0f);
        }
        return Vector3.zero;
