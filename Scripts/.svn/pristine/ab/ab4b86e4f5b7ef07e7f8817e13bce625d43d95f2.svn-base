...
public class TestPersonality : MonoBehaviour
{
    ...
    public IEnumerator Test1()
    {
        ...
        LogHelperLSK.LogError("运行的分钟=" + timespan.TotalMinutes);
        //
        LogHelperLSK.LogError("usableTeams的总数" + usableTeams.Count);
        LogHelperLSK.LogError("aberrantTeams的总数" + aberrantTeams.Count);
    ...
    public IEnumerator Test2()
    {
        ...
        LogHelperLSK.LogError(timespan.TotalMinutes);
    }
    /// <summary>
    /// 第三部
    /// </summary>
    ...
    /// 创建战队队伍
    /// </summary>
    private CombatTeamInfo CreatCombatTeam()
    {
        CombatTeamInfo combatTeamInfo = new CombatTeamInfo(1, TeamType.Player)
        {
            combatUnits = new List<CombatUnit>()
        };
        for (int i = 0; i < 4; i++)
        {
            combatTeamInfo.combatUnits.Add(new CombatUnit(new CharAttribute(new CharCreate(1501)), i));
        }
        return combatTeamInfo;
    }
