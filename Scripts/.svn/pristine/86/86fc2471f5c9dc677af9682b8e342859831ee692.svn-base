...
public class TestCharAttribute : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        //LogHelper_MC.Log("min=" + newRanges[0] + "  max=" + newRanges[1]);
        ...
        //LogHelper_MC.Log("StdDev=" + StdDev);
    ...
    private void TestChar()
    {
        ...
        foreach (int item in temp)
        {
            ...
            //LogHelper_MC.Log("装备instanceID=" + item + "：tempArmor" + equipAttributes[item].tempArmor
    ...
    private void Log(int level, Dictionary<int, CharAttribute> charAttributes, ref List<string> strs)
    {
        ...
        //  LogHelper_MC.Log("//////角色穿戴" + level + "级装备属性//////");
    ...
    private void Test3()
    {
        ...
        LogHelper_MC.Log(ComputeNowIndex(45, lists, 0, lists.Count - 1));
    }
    ...
    private void Start()
    {
        ...
        //{
        //    LogHelper_MC.Log(" " + item);
        //}

        // Test3();
        //   LogHelper_MC.LogError(RandomBuilder.GetRandom(new int[] { 80, 120 }, new int[] { 72, 108 }));
        // serialization();
        ...
        //  LogHelper_MC.LogError(aMaterial.color);
        //  TestCreateSpine();
    }
    ...
    private IEnumerator CreateSpine()
    {
        ...
        isCreate = false;
        yield return null;
        LogHelper_MC.LogError("time=" + time);
    }
    ...
    private void Test1()
    {
        ...
        //        isUpdate = true;
        //        isOk = false;
        //        LogHelper_MC.LogError("计算失败" + grids.First().Key);
        //        continue;
        //    }
        //    _column++;
        ...
        //}
        //if (!isOk) return;
        //LogHelper_MC.LogWarning("计算次数" + _column);
    ...
    private void GetGrids(Dictionary<int, List<int>> _relations)
    {
        ...
        LogHelper_MC.LogError("生成出错" + _sum);
    }
...
//        //得到现在可以用的格子索引
//        _nowUsableIndexs = GetSameLists(_nowUsableLists);
//        LogHelper_MC.LogError(_startIndex + _call.ToString() + _nowGirdId);
//    }
//    else
//    {
//        LogHelper_MC.LogError(_startIndex + _call.ToString() + _nowGirdId + "备份表包含" + _nowGirdId + gridIndexBackups.ContainsKey(_nowGirdId));
//        if (gridIndexBackups.ContainsKey(_nowGirdId))
//        {
//            LogHelper_MC.LogError(_startIndex + _call.ToString() + _nowGirdId + "备份表" + gridIndexBackups[_nowGirdId].Count);
//            _nowUsableIndexs = gridIndexBackups[_nowGirdId];
//        }
//    }
//    //如果没有可以使用的格子
...
//        {
//            LogHelper_MC.LogError(_startIndex + _call.ToString() + "没有备份表");
//            return false;
//            return GetInfo(_startIndex);
//        }
//        //移除前一个格子
//        int _removeId = nowGrids.Last().Key;
//        LogHelper_MC.LogError(_startIndex + _call.ToString() + _nowGirdId + "没有可用的位置");
...
//                    return false;
//                }
//                LogHelper_MC.LogError(_startIndex + _call.ToString() + "备份表为空新建前一个");
//                return GetInfo(_startIndex - 1, false, _startIndex);
//            }
//            //移除最后一个
//            LogHelper_MC.LogError(_startIndex + _call.ToString() + "备份表" + _removeId + "移除之前总数" + gridIndexBackups[_removeId].Count);
...
//                    return false;
//                }
//                LogHelper_MC.LogError(_startIndex + _call.ToString() + "备份表移除当前最后一个位置新建前一个" + (_startIndex - 2));
//                return GetInfo(_startIndex - 1, false, _startIndex);
//            }
//            try
//            {
...
//                LogHelper_MC.LogError(_startIndex + _call.ToString() + _removeId + "备份表选择位置新建前一个OK" + gridIndexBackups[_removeId].Count);
...
//            }
//        }
//        LogHelper_MC.LogError(_startIndex + _call.ToString() + "不包含" + _removeId + "当前备份表重新新建前一个");
...
//    LogHelper_MC.LogError(_startIndex + _call.ToString() + _nowGirdId + "重新新建一个Ok");
